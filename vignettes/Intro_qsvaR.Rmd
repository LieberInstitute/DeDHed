...
```
Next we use the `k_qsvs()` function to calculate how many PCs we will need to account for the variation. A model matrix accounting for relevant variables should be used. Common variables such as Age, Sex, Race and Religion are often included in the model. Again we are using our Ranged Summarized Experiment `DegTx` as the `covComb_tx` option. Next we specify the `mod_tx` with our `model.matrix()`. Again we use the `assayname` option to specify the we are using the "tpm" assay.
