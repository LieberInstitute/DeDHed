---
title: "Introduction to qsva workflow"
author: 
  - name: Your name
    affiliation:
    - Your institution
...
vignette: >
  %\VignetteIndexEntry{Introduction to qsva workflow}
...
`r Biocpkg("qsvaR")` is based on many other packages and in particular in those that have implemented the infrastructure needed for dealing with RNA-seq data (EDIT!). That is, packages like `r Biocpkg("SummarizedExperiment")` (EDIT!).
...
## Create the vignette
library("rmarkdown")
system.time(render("qsvaRexample.Rmd", "BiocStyle::html_document"))

## Extract the R code
library("knitr")
knit("qsvaRexample.Rmd", tangle = TRUE)
```
